<?php
	...
	if(isset($_POST["done"])){
			...
			$res = $tbl->updateObject($field,$values,'username='.format($username,0));
			if($res){
					echo "OK";
				}
		}
...
?>
								<option value="<?php echo $rowPer['id']; ?>" <?php if($rowPer['id']==$row['per']){ echo "selected"; } ?>  ><?php echo $rowPer['name']; ?></option>
<?php
				}
			}
?>
