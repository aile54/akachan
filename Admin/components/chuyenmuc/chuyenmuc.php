<?php
	...
	if(isset($_POST["update"])){
			...
			if(isset($_POST["del"])){
					...
					del_img_array('uploads/chuyenmuc/','chuyenmuc','image',$del);
				}
		}
?>
...
<div id="center-column">
			<div class="top-bar">
				<a href="<?php echo loadPage('addchuyenmuc'); ?>" class="button" ><img src="img/add-folder-icon.png" width="30" height="30" border="0" />  </a>
				<h1>Chuyên mục riêng</h1>
			  <div class="breadcrumbs"><a href="#">Homepage</a> / <a href="#">Chuyên mục riêng</a></div>
			</div><br />
			<!--<form method="post">
...
?>                    
                    <tr>
						<td class="first style1"><a href="<?php echo loadPage('editchuyenmuc&id='.$row['id']); ?>"><?php echo $row['name']; ?></a></td>
                        <td><a href="<?php echo loadPage('editchuyenmuc&id='.$row['id']); ?>"><img src="../<?=$thumb_img?>" height="30" align="absmiddle" /></a></td>
                        <td align="center"><input type="text" value="<?= $row['ordering']; ?>" onkeyup="update_order('ajax/ajax_order.php',<?=$row['id']?>,this.value,'chuyenmuc')" class="txt_order"/></td>
