...
function isTypeAllowed($sFileName)
	{
		$start=strlen($sFileName)-4;
        $duoi=substr($sFileName,$start,4);
		if($duoi=='.swf' || $duoi=='.jpg' || $duoi=='.gif' || $duoi=='.png')
			return true;
		else
			return false;
	/*global $AllowedTypes;
...
function writeFileSelections()
	...
	echo "<div style='overflow:auto;height:222px;width:70%;margin-top:3px;margin-bottom:2px;'>";
		...
		if($sItem=="."||$sItem=="..") 
			...
			if(!$fIsDirectory) 
				...
				if($bDisplay)
					...
					echo "<td valign=top width=50% ><u id=\"idFile".$nIndex."\" style='cursor:pointer;' onclick=\"selectFile(".$nIndex.")\">".$sItem."</u>&nbsp;&nbsp;<img style='cursor:pointer;' onclick=\"downloadFile(".$nIndex.")\" src='download.gif'></td>";
...
<td valign=top style="background:url('bg.gif') no-repeat right bottom;padding-top:5px;padding-left:5px;padding-right:5px;padding-bottom:0px;">
		...
		<td>
				...
				<td nowrap>    
                	<span style="cursor:pointer;"><u><span name="txtLang" id="txtLang" ></span></u></span>&nbsp;
					<span style="cursor:pointer;"><u><span name="txtLang" id="txtLang" ></span></u></span>            
				</td>
...
<tr>
<td class="dialogFooter" style="height:40px;padding-left:550px; width:600px" valign=middle>
